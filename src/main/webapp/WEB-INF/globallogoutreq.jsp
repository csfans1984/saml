	...
	<form action="<%=StringEscapeUtils.escapeHtml(request.getAttribute("Destination").toString()) %>" method="get">
		<table width="100%" border="1">
			...
			<tr>
				...
				<td><%=StringEscapeUtils.escapeHtml(request.getAttribute("RequestXML").toString()) %></td>
			</tr>
			<%
			...
			%>
			<tr>
				<th>SAML Request Code:</th>
				<td><%=StringEscapeUtils.escapeHtml(request.getAttribute("RequestCode").toString()) %><input
					type="hidden" name="SAMLRequest"
					value="<%=StringEscapeUtils.escapeHtml(request.getAttribute("RequestCode").toString()) %>" /></td>
			</tr>
			<%
